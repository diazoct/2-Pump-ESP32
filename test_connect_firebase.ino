//define wifi
#include <WiFi.h>
#define WIFI_SSID "Basement"
#define WIFI_PASSWORD "ontoseno430"
...
// #define soilMoisturePin_5 26
// #define soilMoisturePin_6 25
...
void loop() {
  ...
  int averageSM = (percentageValue1 + percentageValue2 + percentageValue3 + percentageValue4) / 4;
  ...
  if(isPenyiramanMode){
    if (isTimerMode) {
      ...
      if (countdownValue == 0) {
        ...
        digitalWrite(relayPin2, LOW);  // Relay dalam kondisi HIGH
      } else if (countdownValue == -1) {
        digitalWrite(relayPin1, HIGH);
        digitalWrite(relayPin2, HIGH);   // Relay dalam kondisi LOW
      } else {
        digitalWrite(relayPin1, HIGH);
      ...
      if (averageSM <= low) {
        digitalWrite(relayPin1, LOW);
        digitalWrite(relayPin2, LOW); // Aktifkan relay
      ...
      if (strcmp(firebaseData.stringData().c_str(), "on") == 0) {
          digitalWrite(relayPin1, LOW);
          digitalWrite(relayPin2, LOW); // Aktifkan relay manual
      } else {
          digitalWrite(relayPin1, HIGH);
          digitalWrite(relayPin2, HIGH); // Matikan relay manual
  ...
  if(isPestisidaMode){
    ...
    if (strcmp(firebaseData.stringData().c_str(), "on") == 0) {
      ...
      digitalWrite(relayPin4, LOW); // Aktifkan relay manual
    } else if(strcmp(firebaseData.stringData().c_str(), "off") == 0) {
      digitalWrite(relayPin3, HIGH);
      digitalWrite(relayPin4, HIGH); // Matikan relay manual
    }
  } else {
    digitalWrite(relayPin3, HIGH);
